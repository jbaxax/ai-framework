@echo off
rem Windows entry point for the fw pass hook. Claude Code runs hook commands
...
rem
rem Stdin carries the Stop payload and stdout must be JSON or nothing at all,
rem so nothing here may print. A missing Git Bash exits 0 in silence: a broken
rem guard must never break the turn.
setlocal
